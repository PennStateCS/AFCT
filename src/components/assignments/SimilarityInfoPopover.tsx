...
const WHAT_THIS_MEANS: Record<DisplayMatchType, string> = {
  'byte-identical':
    'These submitted files are identical at the byte level. No normalization was applied to this comparison: not the same work, not the same file once formatting is set aside, the same bytes. Two people building the same machine independently produce files that differ somewhere, in a name, a coordinate or the order things were saved in. Because nothing was normalised to reach it, this is the one observation the common-answer threshold does not set aside, even when much of the class shares the file; where that happens the card says so beside it. This is the strongest statement this page can make about two files. What it means about how they came to be identical is for you to judge.',
  exact:
