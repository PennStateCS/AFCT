...
export function EditGroupDialog({ open, setOpen, group, courseId, onSuccess }: EditGroupDialogProps) {
  const defaults: UpdateGroupInput = { name: group?.name ?? '' };

  const { control, handleSubmit, reset, formState: { errors, isSubmitting, isValid } } = useForm<UpdateGroupInput>({
  ...
  const onSubmit = async (raw: UpdateGroupInput) => {
    if (!group) return;
    try {
      const res = await fetch(`/api/courses/${courseId}/groups/${group.id}`, {
        method: 'PATCH',
      ...
      <DialogContent className="bg-card max-w-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <DialogFooter className="bg-card mt-4">
            ...
            <Button type="submit" disabled={!isValid || isSubmitting}>{isSubmitting ? 'Saving…' : 'Save Changes'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
