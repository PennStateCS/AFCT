  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // bg-card, not bg-background: `--background` is the page grey (oklch 0.94) while
          // `--card` and `--popover` are white, so the shadcn default rendered every modal in the
          // page colour while it floated above the page. Forty-three call sites had already
...
function DialogTitle({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title
      ...
      // `leading-tight` rather than `leading-none`: a line box exactly the font size clips
      // descenders, so a title containing g, y or p loses its tail.
