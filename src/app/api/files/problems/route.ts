...
export async function GET(request: NextRequest){
	try {
		...
		if (!user || !['ADMIN', 'FACULTY', 'TA'].includes(user.role)){ //Make sure user is in a valid session
		...
		const filePath = path.join(process.cwd(), 'private', 'uploads', 'problems', fileName);
		...
		console.error('Problem serve error: ', err);
