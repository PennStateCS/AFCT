  ...
  async (req, ctx, { user, courseId }) => {
    ...
    try {
      ...
      if (groupId === 'ALL') {
        ...
        await prisma.groupAssignmentProblem.deleteMany({
          where: { assignmentId, problemId: { in: problemIds } },
        });
      } else if (groupId) {
        // Remove mappings for the specific group
        await prisma.groupAssignmentProblem.deleteMany({
          where: { assignmentId, problemId: { in: problemIds }, groupId },
        });
      } else {
