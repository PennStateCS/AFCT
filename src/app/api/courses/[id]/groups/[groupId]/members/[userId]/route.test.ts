...
describe('DELETE /api/courses/[id]/groups/[groupId]/members/[userId]', () => {
  it('returns 400 when missing params', async () => {
    ...
    const res = await DELETE(new NextRequest('http://localhost/api/courses//groups//members/'), { params: Promise.resolve({ id: '', groupId: '', userId: '' }) } as any);
  ...
  it('returns 404 when membership not found', async () => {
    ...
    const res = await DELETE(new NextRequest('http://localhost/api/courses/c1/groups/g1/members/u2'), { params: Promise.resolve({ id: 'c1', groupId: 'g1', userId: 'u2' }) } as any);
