...
const richJson = {
  version: 1,
  document: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'rich body' }] }] },
};

describe('resolveDescription', () => {
  it('returns plain text for a PLAIN_TEXT record', () => {
    const r = resolveDescription({ description: 'hi', descriptionFormat: 'PLAIN_TEXT', descriptionJson: null });
    expect(r).toEqual({ format: 'PLAIN_TEXT', plainText: 'hi' });
  });

  it('treats a null description as empty', () => {
    const r = resolveDescription({ description: null, descriptionFormat: 'PLAIN_TEXT', descriptionJson: null });
  ...
  it('returns the validated document and derived plain text for a valid TIPTAP_JSON record', () => {
    const r = resolveDescription({ description: 'rich body', descriptionFormat: 'TIPTAP_JSON', descriptionJson: richJson });
  ...
  it('falls back to plain text when the JSON is malformed / unsupported', () => {
    ...
    const r = resolveDescription({ description: 'fallback', descriptionFormat: 'TIPTAP_JSON', descriptionJson: bad });
  ...
  it('falls back when format says TIPTAP_JSON but json is missing', () => {
    const r = resolveDescription({ description: 'fallback', descriptionFormat: 'TIPTAP_JSON', descriptionJson: null });
    expect(r).toEqual({ format: 'PLAIN_TEXT', plainText: 'fallback' });
  });

  it('ignores stray json when the format is PLAIN_TEXT', () => {
    const r = resolveDescription({ description: 'plain', descriptionFormat: 'PLAIN_TEXT', descriptionJson: richJson });
