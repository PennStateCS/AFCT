...
describe('richDescriptionToPlainText', () => {
  it('joins paragraphs with newlines', () => {
    expect(richDescriptionToPlainText(doc(
  ...
  it('keeps heading text and drops marks but keeps their text', () => {
    expect(richDescriptionToPlainText(doc(
      ...
      { type: 'paragraph', content: [{ type: 'text', text: 'strong', marks: [{ type: 'bold' }] }] },
    ))).toBe('Big\nstrong');
  });
  ...
  it('preserves LaTeX source for inline and block math', () => {
    expect(richDescriptionToPlainText(doc(
      { type: 'paragraph', content: [{ type: 'text', text: 'value ' }, { type: 'inlineMath', attrs: { latex: 'x^2' } }] },
    ))).toBe('value $x^2$');
    expect(richDescriptionToPlainText(doc({ type: 'blockMath', attrs: { latex: '\\int_0^1 x' } }))).toBe('$$\\int_0^1 x$$');
  });
  ...
  it('handles hard breaks, horizontal rules, blockquotes, and links', () => {
    ...
    expect(richDescriptionToPlainText(doc(
      { type: 'paragraph', content: [{ type: 'text', text: 'click', marks: [{ type: 'link', attrs: { href: 'https://a.test' } }] }] },
    ))).toBe('click');
  });
