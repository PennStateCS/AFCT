...
//
// The Similarity tab reads `Submission.contentHash`, which is written when a file arrives.
// Anything submitted earlier has none and would never match anything, so this reads each
// stored file once and fills it in. Safe to run more than once: it only touches rows that
...
require('tsx/cjs');
const { submissionContentHash } = require('../src/lib/similarity/content-hash.ts');
    ...
    for (const submission of batch) {
      ...
      try {
        ...
        if (dryRun) {
          ...
          await prisma.submission.update({ where: { id: submission.id }, data: { contentHash } });
        }
        hashed++;
      } catch (error) {
