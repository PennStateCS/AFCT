...
- Per-course roles are stored in `Roster.role` (enum `CourseRole`): `COURSE_ADMIN`, `FACULTY`, `TA`, `STUDENT`.
- Enrollment mapping:
  - Global `FACULTY` and `ADMIN` are assigned `FACULTY` in a course by default; `COURSE_ADMIN` is assigned explicitly.
  - `TA` -> `TA`.
  - `STUDENT` -> `STUDENT`.

## Permissions & Rules (Server-side)
- Global role changes may only be performed by global `ADMIN` (`PATCH /api/users/:id`).
- Per-course role changes may be performed by **global `ADMIN`** or the course's **`COURSE_ADMIN`** (`PATCH /api/courses/:courseId/roster/:userId`).
- Removing a roster member is allowed for **global `ADMIN`**, **course `COURSE_ADMIN`**, and **FACULTY**, subject to constraints:
  ...
  - **FACULTY** cannot remove `COURSE_ADMIN` or other `FACULTY` members.
  - **COURSE_ADMIN** cannot remove other `COURSE_ADMIN` members.
...
- The **Actions** column is only shown to viewers who can act on roster entries (site `ADMIN`, `COURSE_ADMIN`, or `FACULTY`).
- The previous "Manage" dropdown was replaced with a compact **Edit** control that opens `CourseEditUserDialog`.
  - Site `ADMIN` and course `COURSE_ADMIN` use the dialog to edit per-course role and remove users.
...
- `CourseEditUserDialog` supports:
  ...
  - Deleting the user's profile photo (if viewer is FACULTY/COURSE_ADMIN/ADMIN).
...
2. As a course `COURSE_ADMIN`, change a student's per-course role and verify the API and UI reflect the change.
3. As a faculty member, attempt to remove a `COURSE_ADMIN` or another `FACULTY` member — it should be disallowed by the UI and server.
