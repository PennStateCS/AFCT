...
- Per-course roles are stored in `Roster.role` (enum `CourseRole`): `ADMIN`, `FACULTY`, `TA`, `STUDENT`.
- Enrollment mapping:
  - Global `FACULTY` and `ADMIN` are assigned `FACULTY` in a course by default; `ADMIN` can be assigned explicitly.
  - `TA` -> `TA`.
  - `STUDENT` -> `STUDENT`.
...
- Per-course role changes may be performed by **global `ADMIN`** or the course's **`ADMIN`** (`PATCH /api/courses/:courseId/roster/:userId`).
- Removing a roster member is allowed for **global `ADMIN`**, **course `ADMIN`**, and **FACULTY**, subject to constraints:
  ...
  - **FACULTY** cannot remove `ADMIN` or other `FACULTY` members.
  - **ADMIN** cannot remove other `ADMIN` members.
...
- The **Actions** column is only shown to viewers who can act on roster entries (site `ADMIN`, course `ADMIN`, or `FACULTY`).
- The previous "Manage" dropdown was replaced with a compact **Edit** control that opens `CourseEditUserDialog`.
  - Site `ADMIN` and course `ADMIN` use the dialog to edit per-course role and remove users.
...
- `CourseEditUserDialog` supports:
  ...
  - Deleting the user's profile photo (if viewer is ADMIN/course ADMIN/FACULTY).
...
2. As a course `ADMIN`, change a student's per-course role and verify the API and UI reflect the change.
3. As a faculty member, attempt to remove a `ADMIN` or another `FACULTY` member — it should be disallowed by the UI and server.
