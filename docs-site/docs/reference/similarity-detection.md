...
- **Commonality cannot hide identical bytes.** `matchTypeOf` asks three questions in order:
  the instructor's own file first (the handout explains the match), then whether the whole
  relationship is byte-identical, then the common-answer threshold. The middle one is the
  change: every other check normalises something first, so a high share really can be
  convergence, while nothing normalises a raw file. A byte-identical group therefore never
  returns `common` and never reaches the set-aside list; `MatchCluster.aboveCommonShare`
  carries the fact that it is widely shared so the card can say so as context. A widely shared
  POSTED SOLUTION still reads as `common`, which is what it is.
