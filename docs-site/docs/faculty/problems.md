...
- **View Answer** using the viewer for that problem type. The viewer has an **Open in the viewer** button, which shows the same thing in a window of its own for a machine too large to read comfortably in a panel.
