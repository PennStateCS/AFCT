...
- **Submission status** - one bar per problem showing where each participant's latest submission sits in the evaluation queue: **Completed** (evaluated), **Processing** (being evaluated now), **Pending** (queued, waiting for the autograder), **Failed** (evaluation errored), and **Missing** (no submission yet). This chart is about the autograder queue, not about who is late: a problem nobody has submitted shows as Missing whatever the due date says.
