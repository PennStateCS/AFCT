...
Clicking a state opens a small panel in the top right of the drawing showing its name, whether it is the initial or a final state, and every transition into and out of it. Click the background, or the panel's close button, to dismiss it. The same information for every state at once is under **View → Text representation**, which is also the way to read it without a mouse.
