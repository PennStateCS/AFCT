...
- **View** holds **Fit to window**, which brings the whole machine back on screen after zooming or panning about, and **Text representation**, which opens the machine written out as states and transitions in a window of its own. It also turns the background **Grid** on and off, shows or hides the **JFLAP Notes** the author wrote on the canvas (on by default, turns on **Snap to grid** so that dragging a state lands it on the nearest grid line (off by default, so nothing moves unless you ask), and only drawn in the As drawn layout),.
