...
- **Edit** holds **Undo** and **Redo**, which step back through changes to the arrangement: a state dragged to a new place, or the layout switched. Zoom and panning are not included, since they move the view rather than the machine. It also copies the drawing: **Copy as PNG** for a document or an email, and **Copy as SVG** for a drawing program or a slide, where it stays sharp at any size. To copy the machine as words, open **View → Text representation** and use the **Copy as text** button there.
