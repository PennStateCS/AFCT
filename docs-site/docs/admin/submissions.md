...
The viewer opens in a panel over the page, which suits a quick look. It keeps the grid, the zoom controls and **Fit**; the layout choice, the exports and undo live in the standalone window instead. For a large machine, **Open in the viewer** on that toolbar puts the same viewer in a browser window of its own, where it has the whole screen and can sit on a second monitor. The panel closes as the window opens, so the same file is never showing in two places.
...
Resizing the window, or splitting it, never moves you: the zoom stays as you set it and whatever was in the middle of the view stays in the middle, so you keep looking at the part you were looking at. **Fit** is how you ask for the whole machine back.
